# Reviewer checklist — `step15b-learner-clarity-complete`

**Candidate build:** tag `step15b-learner-clarity-complete`, commit `4c2f8f7`, 2026-09-18.
...
This package **supersedes** the Step-15A package in `qa/expert/step15/`, which itself superseded the Step-13 package in
...
1. Work through the sections below with the captures in `qa/visual/step15b_review/` open (69 captures, listed with hashes in
   ...
   `node qa/expert/step15b/capture_review_set.mjs` and check them against the SHA-256 recorded for each one in
   `review-manifest.json`. To review off this machine, ship `qa/visual/step15b_review/` with the package.
...
- [ ] **13.1** Overall expert decision on the `step15b-learner-clarity-complete` candidate build. — **Status: PENDING**
