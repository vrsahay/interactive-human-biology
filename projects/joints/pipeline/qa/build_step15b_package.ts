// Step 15B: build the hash-backed expert review package for the step15b-learner-clarity-complete candidate build.
//
//   npx tsx pipeline/qa/build_step15b_package.ts
//     -> qa/expert/step15b/review-manifest.json
//     -> qa/expert/step15b/expert-review-status.json   (only if it does not exist: a reviewer's decisions are never overwritten)
//     -> qa/expert/step15b/strings-inventory.json
//
// READ ONLY with respect to the product: it hashes and reads, and writes only inside qa/expert/step15b/. It records no
...
const OUT_DIR = join(ROOT, "qa", "expert", "step15b");
const CANDIDATE_TAG = "step15b-learner-clarity-complete";
...
interface Shot { id: string; durationMs: number; a11yKey: string; camera: Record<string, unknown>; caption: { titleKey?: string; textKey?: string; noteKey?: string }; interaction: { promptKey?: string; check?: { promptKey: string; correctKey: string; incorrectKey: string } }; overlays: { concepts: string[] } }
interface Chapter { id: string; number: string; titleKey: string; shots: Shot[] }
interface Explore { exploreId: string; chapterId: string; jointType: string; status: string; site: string; titleKey: string; instructionKey: string; explanationKey: string; statusKey: string; motion: { kind: string; groups?: string[]; limitsDeg?: Record<string, number[]> }; camera: Record<string, unknown> }
...
// Product paths. Step 15A may add QA tooling and review documents; it may not change any of these, and the manifest
...
const integrity = {
  ...
  productionWebBuild: existsSync(join(ROOT, "dist"))
    ...
    : [],
  reviewCaptures: dir("qa/visual/step15b_review", (f) => f.endsWith(".png") || f === "captures.json"),
  packageFiles: dir("qa/expert/step15", (f) => f !== "review-manifest.json").map((e) => e),
  reviewCapturesNote: "qa/visual/ is git-ignored by repository policy because every capture set is reproducible from its own script. These 69 captures are reproduced by qa/expert/step15b/capture_review_set.mjs (tracked, and hashed in packageFiles) against the built dist of this candidate; the SHA-256 of each one is recorded above, so a reviewer who regenerates them can verify they are looking at the same images. To send the package to an off-machine reviewer, ship qa/visual/step15b_review/ alongside it.",
};
...
const chapters = lesson.chapters.map((c) => ({
  ...
  shots: c.shots.map((s) => ({
    ...
    capture: `qa/visual/step15b_review/${c.number}_${c.id}__${s.id}.png`,
  })),
}));
...
const explores = lesson.explores.map((e) => ({
  ...
  captures: [`qa/visual/step15b_review/explore_${e.exploreId.split(".")[1]}_open.png`, `qa/visual/step15b_review/explore_${e.exploreId.split(".")[1]}_moved.png`],
}));
...
const recall = {
  ...
  steps: lesson.recall.steps.map((s, i) => ({
    ...
    captures: [`qa/visual/step15b_review/recall_${i + 1}_question.png`, `qa/visual/step15b_review/recall_${i + 1}_answered.png`],
  })),
};
...
const supportingDocuments = [
  ...
  "qa/step14/narration-coverage.md",
  "qa/step14/camera-direction-review.md",
  "qa/step14/recall-challenge-review.md",
  "qa/step14/explore-matrix.md",
  "qa/step14/teaching-methodology-review.md",
  "qa/step14/step14b-regression.md",
  "qa/step14/step14b-status.json",
  "qa/expert/frozen-build.json",
  "qa/expert/review-status.json",
].map((f) => entry(f));
...
const manifest = {
  ...
  generatedAt: new Date().toISOString(),
  generator: "pipeline/qa/build_step15b_package.ts",
  candidate: identity,
  supersedes: {
    package: "qa/expert/step15/ (Step 15A), which itself superseded qa/expert/ (Step 13)",
    reason: "Step 15B changed what a learner reads and sees: the repeated technical disclaimer left the captions for a persistent status and the Sources dialog, the anatomical names left the teaching sentence for the labels, axis is now explained before it is named, the pivot chapter gained a beat that shows the two bones, flexion and extension are reused, and the one silent transition now speaks. Earlier packages describe builds that are no longer the candidate; each is retained unchanged as the record of what it described.",
  },
  ...
  recall,
  narrationCoverage,
  strings: { file: "qa/expert/step15b/strings-inventory.json", counts: stringsInventory.counts },
  integrity,
  supportingReports: reports,
  supportingDocuments,
  howToUse: "Open reviewer-checklist.md and work through it with the captures in qa/visual/step15b_review/. Record every decision in expert-review-status.json. A passing automated test is evidence, not approval: no item may be moved off PENDING because a test passed.",
};
    ...
    JSON.stringify(
      {
        ...
        document: `Expert decisions for the ${CANDIDATE_TAG} candidate build. Every item starts PENDING and may only be changed by the named human reviewer who made the decision. No automated test result may move an item off PENDING.`,
        commit,
