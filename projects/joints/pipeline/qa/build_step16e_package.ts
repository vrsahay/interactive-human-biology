// Step 16E: build the hash-backed expert review package for the Step-16E candidate build.
//
//   node pipeline/qa/build_step16e_package.ts --candidate <tag>
//     -> qa/expert/step16e/review-manifest.json
//     -> qa/expert/step16e/expert-review-status.json   (only if it does not exist: a reviewer's decisions are never overwritten)
//     -> qa/expert/step16e/strings-inventory.json
//
// READ ONLY with respect to the product: it hashes and reads, and writes only inside qa/expert/step16e/. It records no
...
const OUT_DIR = join(ROOT, "qa", "expert", "step16e");
...
const integrity = {
  ...
  productionWebBuild: existsSync(join(ROOT, "dist"))
    ...
    : [],
  reviewCaptures: dir("qa/visual/step16e_review", (f) => f.endsWith(".png") || f === "captures.json"),
  packageFiles: dir("qa/expert/step16e", (f) => f !== "review-manifest.json"),
  reviewCapturesNote: "qa/visual/ is git-ignored by repository policy because every capture set is reproducible from its own script. These captures are reproduced by qa/expert/step16e/capture_review_set.mjs (tracked, and hashed in packageFiles) against the built dist of this candidate; the SHA-256 of each one is recorded above, so a reviewer who regenerates them can verify they are looking at the same images. To send the package to an off-machine reviewer, ship qa/visual/step16e_review/ alongside it.",
};
...
const chapters = lesson.chapters.map((c) => ({
  ...
  shots: c.shots.map((s) => ({
    ...
    capture: `qa/visual/step16e_review/${c.number}_${c.id}__${s.id}.png`,
  })),
}));
...
const recall = {
  ...
  steps: lesson.recall.steps.map((s, i) => ({
    ...
    captures: [`qa/visual/step16e_review/recall_${i + 1}_question.png`, `qa/visual/step16e_review/recall_${i + 1}_answered.png`],
  })),
};
...
const manifest = {
  ...
  recall,
  narrationCoverage,
  strings: { file: "qa/expert/step16e/strings-inventory.json", counts: stringsInventory.counts },
  integrity,
  supportingReports: reports,
  supportingDocuments,
  howToUse: "Open reviewer-checklist.md and work through it with the captures in qa/visual/step16e_review/. Record every decision in expert-review-status.json. A passing automated test is evidence, not approval: no item may be moved off PENDING because a test passed.",
};
    ...
    JSON.stringify(
      {
        ...
        draftEnrichmentStrings: "PENDING (the 125 draft-enrichment strings a learner can reach, listed in strings-inventory.json; the 126th in the locale, hinge.axis_note, is unreferenced - see reviewer-checklist.md 2.9)",
        mobileExploreComposition: "PENDING (Step 16E: the learner-facing result, reviewed on a real phone)",
