// Step 15A: build the hash-backed expert review package for the `step14b-complete` candidate build.
//
//   npx tsx pipeline/qa/build_step15_package.ts
//     -> qa/expert/step15/review-manifest.json
//     -> qa/expert/step15/expert-review-status.json   (only if it does not exist: a reviewer's decisions are never overwritten)
//     -> qa/expert/step15/strings-inventory.json
//
// READ ONLY with respect to the product: it hashes and reads, and writes only inside qa/expert/step15/. It records no
...
const OUT_DIR = join(ROOT, "qa", "expert", "step15");
const CANDIDATE_TAG = "step14b-complete";
...
const integrity = {
  ...
  productionWebBuild: existsSync(join(ROOT, "dist"))
    ...
    : [],
  reviewCaptures: dir("qa/visual/step15", (f) => f.endsWith(".png") || f === "captures.json"),
  packageFiles: dir("qa/expert/step15", (f) => f !== "review-manifest.json").map((e) => e),
  reviewCapturesNote: "qa/visual/ is git-ignored by repository policy because every capture set is reproducible from its own script. These 69 captures are reproduced by qa/expert/step15/capture_review_set.mjs (tracked, and hashed in packageFiles) against the built dist of this candidate; the SHA-256 of each one is recorded above, so a reviewer who regenerates them can verify they are looking at the same images. To send the package to an off-machine reviewer, ship qa/visual/step15/ alongside it.",
};
...
const chapters = lesson.chapters.map((c) => ({
  ...
  shots: c.shots.map((s) => ({
    ...
    capture: `qa/visual/step15/${c.number}_${c.id}__${s.id}.png`,
  })),
}));
...
const explores = lesson.explores.map((e) => ({
  ...
  captures: [`qa/visual/step15/explore_${e.exploreId.split(".")[1]}_open.png`, `qa/visual/step15/explore_${e.exploreId.split(".")[1]}_moved.png`],
}));
...
const recall = {
  ...
  steps: lesson.recall.steps.map((s, i) => ({
    ...
    captures: [`qa/visual/step15/recall_${i + 1}_question.png`, `qa/visual/step15/recall_${i + 1}_answered.png`],
  })),
};
...
const manifest = {
  ...
  generatedAt: new Date().toISOString(),
  generator: "pipeline/qa/build_step15_package.ts",
  candidate: identity,
  supersedes: {
    package: "qa/expert/ (Step 13)",
    reason: "Steps 14 and 14B changed the learner experience: the lesson went from 6 chapters to 9, gained a universal Explore system and a retrieval challenge, and gained narration for all of it. The Step-13 package describes a build that is no longer the candidate. It is retained unchanged as the record of what was frozen then.",
  },
  ...
  recall,
  narrationCoverage,
  strings: { file: "qa/expert/step15/strings-inventory.json", counts: stringsInventory.counts },
  integrity,
  supportingReports: reports,
  supportingDocuments,
  howToUse: "Open reviewer-checklist.md and work through it with the captures in qa/visual/step15/. Record every decision in expert-review-status.json. A passing automated test is evidence, not approval: no item may be moved off PENDING because a test passed.",
};
    ...
    JSON.stringify(
      {
        ...
        document: "Expert decisions for the step14b-complete candidate build. Every item starts PENDING and may only be changed by the named human reviewer who made the decision. No automated test result may move an item off PENDING.",
        commit,
